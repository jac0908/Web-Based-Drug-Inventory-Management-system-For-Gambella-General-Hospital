...
<script>
  function isdelete()
  {
   ...
   if(!d)
   {
    alert(window.location='deleteemp.php');
   }
   else
   {
   return false;
...
if(isset($_POST['submit']))
{
					...
					$insert=mysql_query("delete from employee where empid='$_GET[empid]'"); 
		if($insert)
		{
			echo '<Script>alert("employee sucessfully deleted")</script>';
...
$query1=mysql_query("select * from employee where empid='$_GET[empid]'");
...
<tr><td colspan="2" ><p><font size="6" color="#045349"> Delete Employee</font> </p></td></tr>
<tr><td><b>Employee ID.:</b></td><td>
<input name="id" type="text" readonly="readonly"value="<?php echo $query2['empid']; ?>"  size="25"style=" margin-bottom: 10px;width: 180px;height: 30px;border:1px Solid #071f56;"/></td></tr>
...
<td><input type="submit" name="reset" value="Reset"style="background-color: red;color: #fffbfb;height: 40px;width:100px;font-size: 20px"></tr>
</table>
</form>
<?php
}
