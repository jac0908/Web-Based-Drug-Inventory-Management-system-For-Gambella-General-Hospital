...
<form id="form1" name="login" method="POST" action="searchitem.php" onsubmit="return formValidation()">
...
<!--Php Script-->  
	...
	if($id=="")
	{
		echo '<p class="wrong"> please enter drug id to search</p>';
			...
			$view=mysqli_query($conn,"select * from drug where drug_id='$id'");
					$rowCheck = mysqli_num_rows($view);
if($rowCheck<1)
{
echo'<p class="wrong">The drug is not found</p>';
echo' <meta content="5;searchitem.php" http-equiv="refresh" />';
}
else
{
